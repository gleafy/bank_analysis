...
from src.utils import fetch_currency_rates, fetch_stock_prices, filter_by_date, get_greeting, get_user_settings


def test_filter_by_date() -> None:
    df = pd.DataFrame({"Дата операции": pd.to_datetime(["2021-05-01", "2021-05-15", "2021-06-01"])})
...
def test_get_user_settings(tmp_path: Any) -> None:
    ...
    settings_path.write_text(json.dumps({"user_currencies": ["USD"], "user_stocks": ["AAPL"]}), encoding="utf-8")
...
def test_fetch_currency_rates(mock_env: Any, mock_get: Any) -> None:
    mock_get.return_value.json.return_value = {"rates": {"USD": 0.02, "EUR": 0.01}}

    result = fetch_currency_rates(["USD", "EUR"])
    assert result == [{"currency": "USD", "rate": 50.0}, {"currency": "EUR", "rate": 100.0}]
