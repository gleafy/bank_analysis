...
from .utils import fetch_currency_rates, fetch_stock_prices, filter_by_date, get_greeting, get_user_settings, load_data
...
def main_page_view(date_str: str) -> dict:
    ...
    for card, group in df_filtered.groupby("Номер карты"):
        ...
        cards_info.append({"last_digits": str(card)[-4:], "total_spent": round(total, 2), "cashback": cashback})
